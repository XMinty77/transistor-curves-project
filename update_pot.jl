...
const UPDATE_WAIT_IBB_TOL_SDEV   = 1.2  # Maximum standard deviation allowed in order to consider a set of Ibb values stable
...
const UPDATE_ADD_IBB_DELTA_NOISE = 8    # Tolerated noise level for IBB

const UPDATE_ADD_IBB_DRIFT_TOLER = 10   # Tolerated unintetional drift for IBB before asking the user to restabilize it
...
function update_add()
    ...
    if (abs(update_ibb_equ - ibb) >= UPDATE_ADD_IBB_DELTA_NOISE && abs(ibb - update_ibb_mem[end - 1]) >= UPDATE_ADD_IBB_DELTA_NOISE * 2)
        ...
        if (abs(update_vcc_mem[end - 1] - vcc) >= UPDATE_ADD_VCC_DELTA_NOISE * 2)
            # Doesn't seem intentional, the user is changing VCC
            if (abs(update_ibb_equ - ibb) > UPDATE_ADD_IBB_DRIFT_TOLER)
                # The drift in IBB is too big now, we should ask the user to readjust to the nominal curve value
                println("Base current changed from $update_ibb_equ but base voltage did not change.\nPlease adjust the base voltage accordingly:")
                global update_state = update_drift
            end
        else
...
function update_wait()
    ...
    if (sdev <= UPDATE_WAIT_IBB_TOL_SDEV)
        ...
        if (abs(update_ibb_equ_old - update_ibb_equ) < UPDATE_WAIT_IBB_TOL_GOBACK)
            println("\nBase current stabilized @ previous value of $update_ibb_equ_old, resuming...")
        ...
        println("\nBase current stabilized @ new value of $update_ibb_equ, drawing a new curve...")
        global update_state = update_add
...
# Waiting for user state but bad, waits until Ib is stablilized back to its equilibrium value
function update_drift()
    ...
    # Continuously display base current in the terminal
    print("\rCurrent base current delta: $(ibb - update_ibb_equ) \t Mean: $(mean(update_ibb_mem))")
    if (abs(ibb - update_ibb_equ) < UPDATE_DRIFT_IBB_DELTA_VALUE) print(" (Hold...)") end
    print("                    ") # Clear rest of the line
    
    if (abs(mean(update_ibb_mem) - update_ibb_equ) <= UPDATE_DRIFT_IBB_DELTA_MEANS && std(update_ibb_mem) <= UPDATE_WAIT_IBB_TOL_SDEV)
        # We are back into roughly the same base current, we can go back
        println("\nCurrent base stabilized @ original value of $update_ibb_equ...")
        global update_state = update_add
        global update_badwait_ticking = false
    end
end